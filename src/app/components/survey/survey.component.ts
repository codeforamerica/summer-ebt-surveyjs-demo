...
import { inputmask, autocomplete } from 'surveyjs-widgets';
import { PanelDynamicQuestionComponent, SurveyModule } from 'survey-angular-ui';
...
export class SurveyComponent {
  ...
  ngOnInit() {
    ...
    survey.getAllQuestions().forEach(q => {     
      if(q.getType() === "paneldynamic") {
        ...
        SurveyComponent.setVisibleDynamicPanel((q as SurveyCore.QuestionPanelDynamicModel),q['startItemIndex']);  
    ...
    survey.onComplete.add(this.saveRawSurveyData);
  ...
  onDynamicPanelAdded(sender: SurveyCore.Model, options: SurveyCore.DynamicPanelModifiedEvent) {
    sender.currentPage = options.question['goToPageOnAddorEdit'];
    sender.getAllQuestions().forEach(q => {     
      if(q.getType() === "paneldynamic") 
      SurveyComponent.setVisibleDynamicPanel((q as SurveyCore.QuestionPanelDynamicModel),options.question.panels.length);         
    });
  }

  onDynamicPanelItemValueChanged(sender: SurveyCore.Model, options: SurveyCore.DynamicPanelItemValueChangedEvent) {
    if (options.name == "edit" && options.value && options.question['goToPageOnAddorEdit'].length > 0)
    {
      ...
      sender.getAllQuestions().forEach(q => {     
        if(q.getType() === "paneldynamic") 
        SurveyComponent.setVisibleDynamicPanel((q as SurveyCore.QuestionPanelDynamicModel),options.panelIndex + 1);         
      });
    }
  }

  onGetPanelFooterActions(sender: SurveyCore.Model, options: SurveyCore.GetPanelFooterActionsEvent) {
        ...
        action: () => {
          sender.getAllQuestions().forEach(q => {     
            if(q.getType() === "paneldynamic") {
              ...
              SurveyComponent.setVisibleDynamicPanel((q as SurveyCore.QuestionPanelDynamicModel), panelIndex + 1);         
            }
           });
          }
      })
  ...
  doMarkdown(sender: any, options: any) {
  ...
  saveRawSurveyData(sender: any, options: any){
    /*
    var fs = require('fs');
    fs.writeFile("test.txt", sender.data, function(err: any) {
      if (err) {
          console.log(err);
      }
    });
    */
    var theJSON = JSON.stringify(sender.data);
  ...
  savePdf(sender: any, options: any) {
    ...
    surveyPdf.onTextMarkdown.add(this.doMarkdown);
